...
def image_pre_process(img, test_size=224):
    img = functional.resize(img, [int(256 / 224 * test_size), int(256 / 224 * test_size)])
    img = functional.center_crop(img, output_size=[test_size, test_size])
    return img
...
def load_image_as_tensor(path, normalize=False, size=None, pre_process=None, crop=False):
    ...
    if size is not None:
        if crop:
            ...
            img = functional.resize(img, size, interpolation=Image.BICUBIC)
