...
from timm.models.helpers import build_model_with_cfg, named_apply, adapt_input_conv, load_checkpoint
...
class VisionTransformer(nn.Module):
    ...
    """
    layers = tuple(['Layer{}'.format(i) for i in range(13)])
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, representation_size=None, distilled=False,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., embed_layer=PatchEmbed, norm_layer=None,
                 act_layer=None, weight_init='', no_epoch=0, layers=None, **kwargs):
        """
        Args:
    ...
    def forward_features(self, x):
        ...
        if self.dist_token is None:
            ...
            return x[:, 0], x[:, 1]
        # self.images = torch.zeros((100,3,224,224)).cuda()
    ...
    def forward(self, x, preprocess=None):
        ...
        feats = []
        for block_idx, layer in enumerate(self.blocks):
            x = layer(x)
            # if 'Layer{}'.format(block_idx) in self.layers:
            #     feats.append(x.view(x.shape[0], -1))

        x1 = self.norm(x)
        xf = self.pre_logits(x1[:, 0])
        feats.append(xf)
        y = self.head(xf)
        feats.append(y)
        return tuple(feats)
...
def vit_tiny_patch16_224(pretrained=True, **kwargs):
    ...
    if pretrained:
        weight_path = osp.join(kwargs['args'].torch_root, 'checkpoints', 'Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_224.npz')
...
def VIT(method, model_name, **kwargs):
